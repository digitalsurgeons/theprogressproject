...
const storageServiceAccount = path.join(__dirname, '../storageKey.json')
const _ = {
...
const storage = new Storage({
  keyFilename: storageServiceAccount,
...
module.exports = (answers, cb) => {
  ...
  async function uploadFile(asset) {
    await storage
      ...
      .upload(`img/${asset}`, {
        destination: `impact-effort/${asset}`,
        gzip: true,
        metadata: {
  ...
  return chartNode
    ...
    .then(() => {
      setTimeout(() => uploadFile(asset).catch(console.error), 1000)
      return `https://storage.googleapis.com/${process.env.GOOGLE_STORAGE_BUCKET}/impact-effort/${asset}`
    })
    .catch((err) => {
      console.log(err)
      return
